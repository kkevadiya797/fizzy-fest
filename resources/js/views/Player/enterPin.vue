<template>
    <div class="container-fluid p-0 h-100">
        <div
            class="fz-name-score bg-white text-center py-2 px-5 position-relative card fz-header-play"
        >
            <img
...
export default {
    ...
    methods: {
        async send() {
            ...
            try {
                await axios
                    .get("/api/checkPin/send/" + this.pin)
                    .then(response => {
                        if (response.data) {
                            this.$emit("next", "enter-name", response.data);
                        }
                    });
            } catch (error) {
                console.log(error);
